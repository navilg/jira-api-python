...
def get_worklogs_in_date_range(username, start_date, end_date):
    ...
    auth = (JIRA_AUTH_USERNAME, JIRA_API_TOKEN)
    params = {
        "jql": jql_query,
        ...
        try:
            ...
            for log in worklogs:
                ...
                if log["author"]["name"] == username and start_datetime <= log_date <= end_datetime:
                    total_hours += log["timeSpentSeconds"] / 3600  # Convert seconds to hours
        except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"Total hours logged by {username} between {start_date} and {end_date}: {total_hours} hours")